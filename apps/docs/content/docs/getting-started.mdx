...
That's it — zero runtime setup needed. tsgonest injects `assertTypeName()` calls for `@Body()` parameters and wraps return statements with `transformTypeName()` at compile time. No pipes, interceptors, or manifest needed.
...
<Cards>
  ...
  <Card title="Serialization & Runtime" description="Compile-time serialization injection" href="/docs/serialization-runtime" />
</Cards>
