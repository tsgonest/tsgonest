...
1. **Validation**: Wraps `@Body()` parameters with `assertTypeName()` calls from the generated companion files
2. **Serialization**: Wraps return statements with `transformTypeName()` calls that use the generated serializers
