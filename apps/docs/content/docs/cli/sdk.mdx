...
# Uses config defaults (sdk.input falls back to openapi.output)
tsgonest sdk
```
...
| --- | --- | --- |
| `--input <path>` | Path to OpenAPI JSON file (legacy) | From config: `sdk.input` or `openapi.output` |
| `--output <path>` | Output directory for generated SDK | From config: `sdk.output` or `./sdk` |
| `--config <path>` | Path to tsgonest config file | auto-detect |
| `--name <name>` | Generate SDK for a specific named OpenAPI output | |

## Config

### Top-level SDK config (legacy)
...
export default defineConfig({
  ...
  sdk: {
    ...
    input: 'dist/openapi.json', // Defaults to openapi.output if omitted
  },
});
```

### Per-output SDK config

When using multiple OpenAPI outputs, configure SDK generation per output:
...
Generate a specific output's SDK:

```bash
tsgonest sdk --name public
```

During `tsgonest build`, per-output SDKs are generated automatically alongside the top-level SDK (if configured).
...
The recommended workflow is to run `tsgonest sdk` after `tsgonest build`:
...
```json title="package.json"
{
  "scripts": {
    ...
    "sdk": "tsgonest sdk",
    "build:all": "tsgonest build && tsgonest sdk"
  }
}
```

Or configure both `openapi.output` and `sdk.output` in your config, then:

```bash
tsgonest build    # Compiles + generates openapi.json
tsgonest sdk      # Reads openapi.json → generates sdk/
```
