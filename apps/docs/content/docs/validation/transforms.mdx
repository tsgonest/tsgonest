...
When using `TsgonestValidationPipe` in NestJS, tsgonest **automatically enables coercion** for `@Query()` and `@Param()` parameters typed as `number` or `boolean`. You do not need to add `Coerce` explicitly for these cases.
