...
| **Standard Schema** | No | Yes — v1 wrappers |
...
| **Config file** | `tsconfig.json` only | `tsconfig.json` + `tsgonest.config.json` |
...
```json title="tsgonest.config.json — minimal (compiler-only mode)"
{
  "transforms": {
    "validation": false,
    "serialization": false
