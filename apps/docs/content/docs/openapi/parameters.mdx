...
tsgonest uses **static analysis** to detect NestJS parameter decorators (`@Body()`, `@Query()`, `@Param()`, `@Headers()`), HTTP method decorators (`@Get()`, `@Post()`, `@Put()`, `@Delete()`, `@Patch()`), and `@Controller()`. If you wrap these core decorators in factory functions, tsgonest cannot see through the indirection and will **not** generate OpenAPI parameters, inject validation, or apply coercion for them.
...
The affected decorators are all core NestJS decorators that tsgonest actively introspects:
...
| Metadata | `@HttpCode`, `@Version`, `@Sse`, `@UseInterceptors`, `@UseGuards` |
...
Custom decorators created with `createParamDecorator` are supported via the `@in` JSDoc tag (see [Custom Decorators](#custom-decorators-with-in) above).
