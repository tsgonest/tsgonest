...
Runtime-generated controllers and dynamic decorator path arguments (e.g., `@Controller(prefix)`, `@Get(dynamicPath)`) are not supported. tsgonest emits a warning and excludes these from OpenAPI output. See [Rules & Limitations](/docs/rules) for full details.
...
- **Tags come from the child** — inherited routes use the child controller's class name and `@tag`
- **Multi-level inheritance** works — `A extends B extends C` collects routes from all ancestors
- **`implements` is ignored** — only `extends` is followed (interfaces have no method bodies)
- **Abstract base classes** work the same as concrete ones
