...
func generateController(ctrl ControllerGroup, doc *SDKDocument, version string) string {
	...
	// Value imports (buildFormData, SSEConnection)
	var valueImports []string
	if hasFormData {
		valueImports = append(valueImports, "buildFormData")
	}
	if hasSSE {
		valueImports = append(valueImports, "SSEConnection")
	}
	if len(valueImports) > 0 {
		sb.WriteString("import { ")
		sb.WriteString(strings.Join(valueImports, ", "))
		sb.WriteString(" } from '")
	...
	// Generate options types (only for methods that need them)
	for _, method := range ctrl.Methods {
	...
	for _, method := range ctrl.Methods {
		sb.WriteString(generateMethodImpl(method))
	}
	sb.WriteString("  };\n")
	sb.WriteString("}\n")
...
// generateMethodImpl generates the implementation of a single SDK method.
func generateMethodImpl(method SDKMethod) string {
	var sb strings.Builder

	hasOptions := hasRequiredOptions(method)
	if hasOptions {
		sb.WriteString(fmt.Sprintf("    async %s(options) {\n", method.Name))
	} else {
		sb.WriteString(fmt.Sprintf("    async %s(options) {\n", method.Name))
	}
	...
	if method.Body != nil {
		if method.Body.ContentType == "multipart/form-data" {
			sb.WriteString("        body: formData,\n")
			sb.WriteString("        contentType: 'multipart/form-data',\n")
		} else {
			sb.WriteString("        body: options.body,\n")
	...
	if method.ResponseContentType != "" && method.ResponseContentType != "application/json" {
		...
		sb.WriteString(fmt.Sprintf("        responseType: '%s',\n", hint))
	}
	sb.WriteString("        signal: options?.signal,\n")
	sb.WriteString("        headers: options?.headers,\n")
	sb.WriteString("      });\n")
	sb.WriteString("    },\n")
