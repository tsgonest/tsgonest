...
func GenerateCompanionSelective(typeName string, meta *metadata.Metadata, registry *metadata.TypeRegistry, includeValidation bool, includeSerialization bool, opts ...CompanionGenOptions) string {
	...
	// Detect external recursive refs that need companion file imports.
	// These are types from other files that are self-recursive, so their
	// is*/validate*/serialize* functions can't be inlined and must be imported.
	externalRefs := collectExternalRecursiveRefs(typeName, meta, registry)
...
func collectExternalRecursiveRefs(typeName string, meta *metadata.Metadata, registry *metadata.TypeRegistry) []companionImport {
	// Track which external recursive refs we've found
	found := make(map[string]bool)
	var result []companionImport
		...
		case metadata.KindRef:
			...
			if generating[m.Ref] {
				...
				if m.Ref != typeName && !found[m.Ref] {
					...
					if importPath != "" {
						result = append(result, companionImport{
							TypeName:   m.Ref,
