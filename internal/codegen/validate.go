...
type CompanionGenOptions struct {
	...
	// SourceFile is the output base path of the source file being generated
	// (e.g., "user-sidebar/dto/unread.dto"). Used to compute relative import
	// paths for cross-companion file references.
	SourceFile string
	// SourceToOutput maps absolute source-file paths (as reported by tsgo's
	// SourceFile.FileName, always forward-slash) to their expected output
	// path with extension preserved (e.g. "/abs/dist/validators/x.ts"). Used
	// to compute relative imports for `Validate<typeof fn>` validator
	// functions whose source file lives outside the current companion's
	// directory. Empty when codegen is invoked directly from tests; in that
	// case the validator's recorded module path is used as a best-effort
	// fallback (this is the path that flows through the test harness).
...
func GenerateCompanionSelective(typeName string, meta *metadata.Metadata, registry *metadata.TypeRegistry, includeValidation bool, includeSerialization bool, opts ...CompanionGenOptions) string {
	...
	var sourceToOutput map[string]string
	rtc := "guard"
	if len(opts) > 0 {
		...
		sourceFile = opts[0].SourceFile
		sourceToOutput = opts[0].SourceToOutput
	}
	e := NewEmitter()
	e.Line("// Auto-generated by tsgonest — do not edit")
	e.Blank()

	// Detect external recursive refs.
	// - `externalRefs` are recursive refs whose helpers live in a separate
	//   companion file and must be imported.
	// - `localHelperRefs` are recursive refs whose types are known but have no
	//   companion file (e.g., types imported from node_modules like
	//   @tiptap/core's JSONContent). For these, helper functions are emitted
	//   locally in this companion file so generated code doesn't reference
	//   undefined identifiers at runtime.
	externalRefs, localHelperRefs := collectExternalRecursiveRefs(typeName, meta, registry)
	externalRefSet := make(map[string]bool, len(externalRefs))
	for _, ext := range externalRefs {
		externalRefSet[ext.TypeName] = true
	}

	// Compute the current companion's path early — used both for cross-companion
	// recursive ref imports and for Validate<typeof fn> validator imports.
	currentCompanionPath := companionPath(sourceFile, typeName)

	if includeValidation {
		// Collect and emit imports for custom validator functions (Validate<typeof fn>).
		// The recorded SourceFile is an absolute, forward-slash path from
		// tsgo (or, in tests, a pre-shaped relative path). Map it to the
		// validator's expected JS output and compute a real relative path
		// from this companion's location, so the emitted specifier loads
		// regardless of build host or deployment root. See #159.
		imports := collectValidateImports(meta, registry)
		for _, imp := range imports {
			module := resolveValidatorImport(currentCompanionPath, imp.SourceFile, sourceToOutput)
			e.Line("import { %s } from %q;", imp.FnName, module)
		}
		if len(imports) > 0 {
			e.Blank()
		}
	...
	// Emit imports for external companion files (cross-companion recursive refs)
	for _, ext := range externalRefs {
		var fns []string
		if includeValidation {
			fns = append(fns, "is"+ext.TypeName, "validate"+ext.TypeName, "assert"+ext.TypeName)
		}
		if includeSerialization {
			fns = append(fns, "serialize"+ext.TypeName)
		}
		if len(fns) > 0 {
			importPath := relativeCompanionImport(currentCompanionPath, ext.ImportPath)
			e.Line("import { %s } from %q;", strings.Join(fns, ", "), importPath)
		}
	}
	...
	// Emit helper functions for recursive refs to types that have no companion
	// file (typically types imported from node_modules, e.g. @tiptap/core's
	// JSONContent). Without these, generated code would call identifiers that
	// were never declared and throw a ReferenceError at runtime.
	for _, lh := range localHelperRefs {
		resolved, ok := registry.Types[lh.TypeName]
		if !ok {
			continue
		}
		if includeValidation {
			isCtx := &validateCtx{
				generating:   map[string]bool{lh.TypeName: true},
				externalRefs: externalRefSet,
			}
			generateIsFunction(e, lh.TypeName, resolved, registry, isCtx)
			e.Blank()

			validateLocalCtx := &validateCtx{
				generating:   map[string]bool{lh.TypeName: true},
				externalRefs: externalRefSet,
			}
			generateValidateFunction(e, lh.TypeName, resolved, registry, validateLocalCtx)
			e.Blank()

			assertLocalCtx := &validateCtx{
				generating:   map[string]bool{lh.TypeName: true},
				externalRefs: externalRefSet,
			}
			generateAssertFunction(e, lh.TypeName, resolved, registry, assertLocalCtx)
			e.Blank()
		}
		if includeSerialization {
			sLocalCtx := &serializeCtx{
				generating:   map[string]bool{lh.TypeName: true},
				externalRefs: externalRefSet,
			}
			generateSerializeFunction(e, lh.TypeName, resolved, registry, sLocalCtx)
			e.Blank()
		}
	}
...
// SourceFile is the validator's source-file path as recorded in the metadata
// (typically an absolute, forward-slash path from tsgo's SourceFile.FileName,
// possibly with a `.ts`/`.tsx`/`.mts`/`.cts` extension). The relative module
// specifier is computed at emit time so the path can be made relative to the
// companion's actual output location.
				...
				if prop.Constraints != nil && prop.Constraints.ValidateFn != nil && prop.Constraints.ValidateModule != nil {
					...
					if !seen[key] {
						...
						imports = append(imports, validateImport{
							...
							SourceFile: *prop.Constraints.ValidateModule,
						})
					}
				}
				scan(&prop.Type)
...
// stripTSExtension drops a leading TypeScript-family extension from a path.
// JS module specifiers must not include the source extension.
...
// resolveValidatorImport computes the JS module specifier for a `Validate<typeof fn>`
// validator function, given the current companion's output path and the validator's
// recorded source file. The returned specifier is always a forward-slash relative
// path (./foo or ../foo/bar) so it loads the same way under ESM and CJS, on the
// build host and after the artifact is shipped to a different filesystem.
//
// Resolution:
//  1. If sourceToOutput maps the absolute source path, use the mapped output base
//     (i.e. dist/-relative path) as the import target. This is the production
//     path: build.go records every program source file's expected output here.
//  2. Otherwise (validators outside the program, or test harness inputs that are
//     already relative), use the recorded path verbatim as the import target.
//
// Either way the path is fed through `relativeCompanionImport`, which yields a
// real path-relative specifier (and normalizes backslashes — see #140).
func resolveValidatorImport(fromCompanionPath, validatorSourcePath string, sourceToOutput map[string]string) string {
	target := validatorSourcePath
	if sourceToOutput != nil {
		if mapped, ok := sourceToOutput[validatorSourcePath]; ok {
			target = mapped
		}
	}
	target = stripTSExtension(target)
	return relativeCompanionImport(fromCompanionPath, target)
}

// companionImport represents an import needed from an external companion file.
type companionImport struct {
	TypeName string // e.g., "BaseChannelMessageResponse"
	// ImportPath is the relative path to the external companion .tsgonest.js file.
	// Empty means the ref was detected but no companion exists to import from
	// (e.g., a recursive type from node_modules like @tiptap/core's JSONContent).
	// In that case, helper functions must be emitted locally in the current file.
	ImportPath string
}

// collectExternalRecursiveRefs scans the metadata tree for KindRef types that are
// self-recursive but are NOT the type being generated. These need import statements
// because their is*/serialize*/validate* functions live in a different companion file.
//
// Returns two slices:
//   - imports: refs whose helpers live in another companion file and must be imported.
//   - localEmits: refs discovered as recursive whose types are registered but whose
//     source file is unknown (typically external node_modules types). For these,
//     helper functions must be emitted locally inside the current companion file.
func collectExternalRecursiveRefs(typeName string, meta *metadata.Metadata, registry *metadata.TypeRegistry) (imports []companionImport, localEmits []companionImport) {
	// Track which recursive refs we've found
	found := make(map[string]bool)

	// Walk the metadata tree, mimicking how the codegen walks it.
	// We track "generating" the same way the codegen does to detect
	// which refs will trigger function calls instead of inlining.
	generating := map[string]bool{typeName: true}
	...
	scan = func(m *metadata.Metadata) {
		...
		case metadata.KindRef:
			...
			if generating[m.Ref] {
				// This ref will emit a function call. If it's not the type
				// being generated, it's an external recursive ref.
				if m.Ref != typeName && !found[m.Ref] {
					found[m.Ref] = true
					importPath := companionImportPath(m.Ref, registry)
					entry := companionImport{TypeName: m.Ref, ImportPath: importPath}
					if importPath != "" {
						imports = append(imports, entry)
					} else if _, ok := registry.Types[m.Ref]; ok {
						// No companion file exists (e.g., type comes from
						// node_modules). Emit helpers locally instead of
						// producing broken references.
						localEmits = append(localEmits, entry)
					}
				}
				return
			}
			// Not yet generating — inline it (same as codegen behavior)
			if resolved, ok := registry.Types[m.Ref]; ok {
				generating[m.Ref] = true
				scan(resolved)
				delete(generating, m.Ref)
			}
		case metadata.KindObject:
			for i := range m.Properties {
				scan(&m.Properties[i].Type)
			}
		case metadata.KindArray:
			if m.ElementType != nil {
				scan(m.ElementType)
		...
		case metadata.KindIntersection:
			for i := range m.IntersectionMembers {
				scan(&m.IntersectionMembers[i])
			}
		}
	}
	scan(meta)
	...
	sort.Slice(imports, func(i, j int) bool {
		return imports[i].TypeName < imports[j].TypeName
	})
	sort.Slice(localEmits, func(i, j int) bool {
		return localEmits[i].TypeName < localEmits[j].TypeName
	})
	return imports, localEmits
}

// companionImportPath computes the import path for an external type's companion file.
// Returns the companion file path (e.g., "./channel/dto/channel.dto.BaseChannelMessageResponse.tsgonest.js")
// based on the source file recorded in the registry.
func companionImportPath(typeName string, registry *metadata.TypeRegistry) string {
	sourceFile := registry.SourceFile(typeName)
	if sourceFile == "" {
		return ""
	}
	// Build the companion file path from source file + type name
	return companionPath(sourceFile, typeName)
}
...
// relPath computes a relative path from basePath to targetPath.
// Returns the relative path and an error string (empty on success).
func relPath(basePath, targetPath string) (string, string) {
	// Clean paths
	basePath = path.Clean(basePath)
	targetPath = path.Clean(targetPath)

	// Split into components
	baseComps := strings.Split(basePath, "/")
	targetComps := strings.Split(targetPath, "/")
	...
	// Find common prefix
	common := 0
	for common < len(baseComps) && common < len(targetComps) && baseComps[common] == targetComps[common] {
		common++
	}

	// Build relative path: go up from base, then down to target
	var parts []string
	for i := common; i < len(baseComps); i++ {
		parts = append(parts, "..")
	}
	parts = append(parts, targetComps[common:]...)
...
type validateCtx struct {
	...
	// externalRefs is the set of type names that are imported from external companion files.
	// When a recursive ref hits one of these, the codegen uses the imported exported function
	// instead of a non-existent local inner function.
	...
	case metadata.KindRef:
		if ctx != nil && ctx.generating[meta.Ref] {
			if ctx.externalRefs[meta.Ref] {
				// External recursive ref — call the imported exported validate function
				// and merge errors with path rewriting.
				fnName := "validate" + meta.Ref
				e.Line("{ const _r = %s(%s); if (!_r.success) for (const _e of _r.errors) errors.push({ ..._e, path: _e.path.replace(/^input/, %s) }); }", fnName, accessor, pathExpr)
			} else {
				// Same-companion recursive call — use inner function with shared errors array
