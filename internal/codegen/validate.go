...
// GenerateValidation generates JavaScript validation, assertion, and deserialization
// functions for a named type.
func GenerateValidation(typeName string, meta *metadata.Metadata, registry *metadata.TypeRegistry) string {
	e := NewEmitter()
	...
	generateDeserializeFunction(e, typeName)
...
// generateDeserializeFunction generates: export function deserialize<Name>(json) { ... }
func generateDeserializeFunction(e *Emitter, typeName string) {
	fnName := "deserialize" + typeName
	assertFn := "assert" + typeName
	e.Block("export function %s(json)", fnName)
	e.Line("const input = JSON.parse(json);")
	e.Line("return %s(input);", assertFn)
	e.EndBlock()
}
...
// GenerateValidationTypes generates TypeScript declaration (.d.ts) content
// for the validation companion file, including Standard Schema v1 type info.
func GenerateValidationTypes(typeName string) string {
	e := NewEmitter()
	e.Line("// Auto-generated by tsgonest — do not edit")
	e.Blank()
	...
	e.Line("export declare function deserialize%s(json: string): %s;", typeName, typeName)
